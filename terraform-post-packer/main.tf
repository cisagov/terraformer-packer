...
# The IDs of all ARM64 cisagov/terraformer-packer AMIs
data "aws_ami_ids" "historical_amis_arm64" {
  ...
  filter {
    name   = "name"
    values = ["terraformer-hvm-*-arm64-ebs"]
  }
...
# The IDs of all x86-64 cisagov/terraformer-packer AMIs
data "aws_ami_ids" "historical_amis_x86_64" {
  ...
  filter {
    name   = "name"
    values = ["terraformer-hvm-*-x86_64-ebs"]
  }
